...
import numpy as np
from .pr2_utils import load_inverse_reachability
...
class Grasp(object):
    ...
    def without_id(self):
        return str(
            f"grasp {self.body} {self.grasp_type} {self.approach} {self.value} {self.carry}"
        )
...
def get_ir_sampler(
    ...
    def gen_fn(arm, obj, pose, grasp, fluents):
        ...
        #         pose.assign()
        #         draw_circle(pose.value[0], 0.1, color=(1, 1, 0.5))
        #         bq.assign()
        #         wait_if_gui()
        ...
        #         yield None
        if learned:
            gripper_from_base_list = load_inverse_reachability(arm, grasp.grasp_type)
            qs = np.zeros((len(gripper_from_base_list), 3))
            # draw_circle(pose.value[0], 0.1, color=(1, 1, 0))
            for i, gripper_from_base in enumerate(gripper_from_base_list):
                base_point, base_quat = multiply(gripper_pose, gripper_from_base)
                x, y, _ = base_point
                _, _, theta = euler_from_quat(base_quat)

                # draw_circle((x, y, 0), 0.2, color=(1, 0, 0))
                qs[i, :] = np.array([x, y, theta])
        lower_limits, upper_limits = get_custom_limits(
            robot, base_joints, custom_limits
        )
        lower_limits = np.array(lower_limits)[None, :]
        upper_limits = np.array(upper_limits)[None, :]
        # draw_circle(pose.value[0], 0.1)
        # wait_if_gui()
        while True:
            if learned:
                np.random.shuffle(qs)
            else:
                radius = np.random.uniform(0.3, 1.0, size=max_attempts)
                theta = np.random.uniform(-np.pi, np.pi, size=max_attempts)
                xs = radius * np.cos(theta) + gripper_pose[0][0]
                ys = radius * np.sin(theta) + gripper_pose[0][1]
                ths = np.random.uniform(-np.pi, np.pi, size=max_attempts)
                qss = np.stack((xs, ys, ths), axis=1)
                idx_valid2 = (lower_limits < qss).all(axis=1)
                idx_valid2 = idx_valid2 & (qss < upper_limits).all(axis=1)
            idx_valid = (lower_limits < qs).all(axis=1)
            idx_valid = idx_valid & (qs < upper_limits).all(axis=1)
            for base_conf in qs[idx_valid]:
                bq = Conf(robot, base_joints, base_conf)
                pose.assign()
                bq.assign()
                set_joint_positions(robot, arm_joints, default_conf)
...
def get_ik_fn(problem, custom_limits={}, collisions=True, teleport=False,
    ...
    def fn(arm, obj, pose, grasp, fluents, obs, base_conf):
        ...
        grasp_conf = pr2_inverse_kinematics(
            ...
            obstacles=obstacles#obstacles + obs
...
def get_ik_ir_gen(problem, max_attempts=25, learned=True, teleport=False, **kwargs):
    ...
    def gen(*inputs):
        b, a, p, g, fluents, obs = inputs
        ir_generator = ir_sampler(b, a, p, g, fluents)
        attempts = 0
        while True:
            if max_attempts <= attempts:
                if not p.init:
